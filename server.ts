...
import { gameService, lobbyService, sessionService } from "./src/lib/services";
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("create-lobby", async (playerName: string) => {
      try {
        ...
        const visitorId = socket.handshake.query.visitorId as
          | string
          | undefined;
        const { id: playerId } = await sessionService.getOrCreatePlayer(
          visitorId,
          playerName
        );
        ...
        // Register socket
        await sessionService.registerSocket(
          socket.id,
          playerId,
          playerName,
          lobby.code
    ...
    socket.on(
      "join-lobby",
      async ({ code, playerName }: { code: string; playerName: string }) => {
        try {
          // Create or get player
          const visitorId = socket.handshake.query.visitorId as
            | string
    ...
    socket.on(
      ...
      async ({ code, playerName }: { code: string; playerName: string }) => {
        try {
          ...
          const reconnectData = await sessionService.getReconnectionData(
            playerId
          );

          // Try to reconnect to existing lobby
          const result = await lobbyService.handlePlayerReconnect(
            code.toUpperCase(),
            playerId,
            socket.id
          );
          ...
          const joinResult = await lobbyService.joinLobby(
            code,
            playerId,
            socket.id,
            playerName
    ...
    socket.on(
      "submit-clue",
      async ({ code, clue }: { code: string; clue: string }) => {
        try {
          if (!currentPlayerId) return;

          const result = await gameService.submitClue(
            code,
            currentPlayerId,
            clue
          );
        ...
        try {
          ...
          if (result.allVotesIn && result.voteResult) {
            if (result.voteResult.isChameleon) {
              ...
              const chameleonPlayer = players.find(
                (p) => p.id === result.voteResult!.mostVotedId
              );
              const lobby = await lobbyService.getLobbyState(code);
    ...
    socket.on(
      "chameleon-guess",
      async ({ code, guess }: { code: string; guess: string }) => {
        try {
          const lobby = await lobbyService.getLobbyState(code);
          if (
            !lobby ||
            !currentPlayerId ||
            currentPlayerId !== lobby.chameleonId
          )
            return;
    ...
    socket.on("disconnect", async () => {
      ...
      try {
        ...
        if (session && currentLobbyCode) {
          ...
          await lobbyService.handlePlayerDisconnect(
            currentLobbyCode,
            session.playerId
          );
          ...
          setTimeout(async () => {
            ...
            if (reconnectData) {
              ...
              if (!result.lobbyDeleted) {
                const lobby = await lobbyService.getLobbyState(
                  currentLobbyCode!
                );

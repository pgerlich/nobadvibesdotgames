...
import type { Player, GameState, GameStartedPayload, VotingPhasePayload, GameResultsPayload, ChameleonGuessPhasePayload } from '@/types/game';
...
export function GameProvider({ children }: { children: ReactNode }) {
  ...
  const [clues, setClues] = useState<Array<{ id: string; name: string; clue: string | null }>>([]);
  
  const [votesCount, setVotesCount] = useState(0);
  const [selectedVote, setSelectedVote] = useState<string | null>(null);
  
  const [gameResults, setGameResults] = useState<GameResultsPayload | null>(null);
  const [guessPhaseData, setGuessPhaseData] = useState<ChameleonGuessPhasePayload | null>(null);
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      ...
      if (session) {
        ...
        newSocket.emit('rejoin-lobby', { code: session.lobbyCode, playerName: session.playerName });
      }
    });
    ...
    newSocket.on('rejoin-success', ({ code, players, state, isHost: hostStatus }) => {
      setLobbyCode(code);
      setPlayers(players);
    ...
    newSocket.on('game-started', (data: GameStartedPayload) => {
      ...
      setClues(data.playerOrder.map(p => ({ id: p.id, name: p.name, clue: null })));
      setGameResults(null);
    ...
    newSocket.on('clue-submitted', ({ playerId, clue }) => {
      setClues(prev => prev.map(c => c.id === playerId ? { ...c, clue } : c));
    });
    ...
    newSocket.on('chameleon-guess-phase', (data: ChameleonGuessPhasePayload) => {
  ...
  const joinLobby = useCallback((code: string, name: string) => {
    setMyName(name);
    socket?.emit('join-lobby', { code: code.toUpperCase(), playerName: name });
  }, [socket]);

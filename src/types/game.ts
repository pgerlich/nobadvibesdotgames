...
export interface ServerToClientEvents {
  ...
  'rejoin-success': (data: { code: string; players: Player[]; state: GameState; isHost: boolean }) => void;
  'rejoin-failed': () => void;
  'game-started': (data: GameStartedPayload) => void;
  'next-player': (data: { currentPlayer: Player; roundEndTime: number }) => void;
  'clue-submitted': (data: { playerId: string; playerName: string; clue: string; clues: any[] }) => void;
